...
public class WorldPacketsEncoder extends Encoder {
	...
	public void sendClanSettings(ClansManager handler, boolean myClan) {
		OutputStream stream = new OutputStream(handler == null ? 4 : handler.getClanSettingsDataBlock().length + 4);
		stream.writePacketVarShort(player, 133);
		stream.writeByte(myClan ? 1 : 0);
		if (handler != null)
		    stream.writeBytes(handler.getClanSettingsDataBlock());
	...
	public void sendClanChannel(ClansManager handler, boolean myClan) {
		OutputStream stream = new OutputStream(handler == null ? 4 : handler.getClanChannelDataBlock().length + 4);
		stream.writePacketVarShort(player, 85);
		stream.writeByte(myClan ? 1 : 0);
		if (handler != null)
		    stream.writeBytes(handler.getClanChannelDataBlock());
		stream.endPacketVarShort();
		session.write(stream);
	}
       ...
       public void sendWorldList(boolean full) {
		...
		if (full) {
			...
			for (int world = 1; world <= WorldList.WORLDS.size(); world++) {
				...
				packet.writeByte(0); //LOC (IDX in List) - KEEP @ "0"
				int flags = 0;
				if (WorldList.WORLDS.get(world).isMembers()) flags |= 0x1;
				flags |= 0x8;//World Includes Loot Share
				packet.writeInt(flags); //Flags: 1-2-4-8 (1=MEMBERS)
				packet.writeGJString(""); //Country
				packet.writeGJString(WorldList.WORLDS.get(world).getIP()); //End Point to Connect to
			}
			packet.writeInt(-1723296702);
		}
		for (int world = 1; world <= WorldList.WORLDS.size(); world++) {
			packet.writeSmart(world); //WID
			packet.writeShort(100); //Player-Size
		}
