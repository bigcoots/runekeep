...
public class WarriorCombat extends CombatScript {
	// blue gfxs - 2854-2855
	// blue ranged - 2853
	...
	// Animations
	// 451 - fall down and stand back up quickly
	// 451 - ranged attack. Throw knives
	// 451 - magic
	...
	public void sprayAttack(final NPC npc, final Entity target) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			@Override
			public void run() {
				npc.setNextGraphics(new Graphics(1935));
	...
	public Object[] getKeys() {
		return new Object[] {13853, 13854, 13855 };
	}
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (!specialRapidMagic && Utils.random(15) == 2 && !sprayAttack
			...
			npc.setNextGraphics(new Graphics(2345));
				...
				public void run() {
					...
					for (Entity t : npc.getPossibleTargets()) {
						...
						delayHit(npc, 1, t, new Hit(npc, Utils.random(150),
								HitLook.MAGIC_DAMAGE));
						t.setNextGraphics(new Graphics(1154, 0, 0));
					}
		...
		if (attackStyle == 1 && !specialRapidMagic && !sprayAttack) { // range
			npc.setNextAnimation(new Animation(451, 20));
			...
			npc.setNextAnimation(new Animation(451));
			npc.setNextGraphics(new Graphics(1639));
			for (Entity t : npc.getPossibleTargets()) {
				if (!t.withinDistance(npc, 11))
					continue;
				delayHit(
				...
				t.setNextGraphics(new Graphics(659, 20, 0));
			}
		}
		return 5;
	}
