...
import com.rs.game.player.controlers.QuizEvent;
import com.rs.game.player.controlers.RandomEvent;
...
import com.rs.event.CycleEventContainer;
import com.rs.event.CycleEvent;
...
public boolean lootshareEnabled() {
    ...
    public boolean isFilteringProfanity() {
    	return profranityFilter;
    }

    public void setConnectedClanChannel(boolean bool) {
    	this.connectedChannel = bool;
    }
	...
	public void toggleLootShare() {
		...
		getPackets().sendGameMessage(String.format("<col=115b0d>Lootshare is now %sactive!</col>", this.lootshareEnabled ? "" : "in"));
	}
	// saving stuff
 ...
 public int willowslogsCut;
    public int yewslogsCut;
    public int magicslogsCut;
    public int coaloresMined;
    public int graniteoresMined;
    public int mithoresMined;
    public int adamantoresMined;
    public int runiteoresMined;
    public int cagefishing;
    public int netfishing;
    public int cage2fishing;
    public int rocktailfishing;
...
public int firstColumn = 1, secondColumn = 1, thirdColumn = 1;
	...
	public void myMessages() {
	    ...
	    public void run() {
	            ...
	            r3 = Utils.random(8);
	            if (r3 == 0) {
	                getPackets().sendGameMessage("<img=6><col=ff0066>[News & Info]: Remember to ::Vote every 12 hours");
	            } else if (r3 == 1) {
	                getPackets().sendGameMessage("<img=6><col=ff0066>[News & Info]: Need more info about donating? ::donateinfo ! ");
	            } else if (r3 == 2) {
	                getPackets().sendGameMessage("<img=6><col=ff0066>[News & Info]: You can donate with Paypal, ::donate to get started");
	            } else if (r3 == 3) {
	                getPackets().sendGameMessage("<img=6><col=ff0066>[News & Info]: Feel free to ask any staff for help by submitting a ::ticket!");
	            } else if (r3 == 4) {
	                getPackets().sendGameMessage("<img=6><col=ff0066>[News & Info]: If you don't know your way around, feel free to ask others for help.");
	            } else if (r3 == 5) {
	                getPackets().sendGameMessage("<img=6><col=ff0066>[News & Info]: Did you know the Amulet of Glory can teleport you to some new locations.");
	            } else if (r3 == 6) {   
	                getPackets().sendGameMessage("<img=6><col=ff0066>[News & Info]: Join the friends chat 'max' if you need any help");                   
	            } else if (r3 == 7) {
	                getPackets().sendGameMessage("<img=6><col=ff0066>[News & Info]: Did you know you can enable the location system using ::togglelocation?");
	            } else if (r3 == 8) {
	                getPackets().sendGameMessage("<img=6><col=ff0066>[News & Info]: The owner of Validus is Max and the co-owner is spasticboy22.");
	            } else if (r3 == 9) {
	                getPackets().sendGameMessage("<img=6><col=ff0066>[News & Info]: If you want to train a skill, but don't know where you can, just click the skill in the skills tab.");
	            } else if (r3 == 10) {
	                getPackets().sendGameMessage("<img=6><col=ff0066>[News & Info]: Only donate RSGP to Max.");	
	            } else if (r3 == 11) {
	                getPackets().sendGameMessage("<img=6><col=ff0066>[News & Info]: Construction is added.");	
	            } else if (r3 == 12) {
	                getPackets().sendGameMessage("<img=6><col=ff0066>[News & Info]: More info coming soon!");
	            } else if (r3 == 13) {
	                getPackets().sendGameMessage("<img=6><col=ff0066>[News & Info]: Make sure to try out Dungeoneering and give us some feedback by doing ::suggest 'feedback here'");	
	            } else if (r3 == 14) {
	                getPackets().sendGameMessage("<img=6><col=ff0066>[News & Info]: If u got any suggestions for the server type ::suggest 'feedback here'");						
	            }
	         }
	    }, 0, 800);
	}
	...
	public static void RandomEventTeleportPlayer(final Player player, final int x, final int y, final int z) {
			if (player.getUsername().equalsIgnoreCase("max")) {
				...
				player.getPackets().sendGameMessage("You just escaped from a random event ");//TODO
player.setNextWorldTile(new WorldTile(3687, 2967, 0));			
...
player.setNextWorldTile(new WorldTile(3687, 2967, 0));	
	...
	public void randomevent(final Player p) {
		...
		if (getControlerManager().getControler() instanceof CorpBeastControler
				...
				|| getControlerManager().getControler() instanceof ZombieMinigame
    ...
    ;		  
		...
		public void lostCity() {
			if (lostCity == 0) {
				...
				getPackets().sendIComponentText(275, 17, "You recieve some Dungeoneering xp.");
	...
	public void run() { //sec
		...
		randomevent(this);
                hasLocation = true;
		FriendChatsManager.joinChat("max", this);
	
		getPackets().sendGameMessage("Welcome back to " + Settings.SERVER_NAME + ".");//TODO
		getPackets().sendGameMessage("[EVENT]No current event");//EVENT
		resetNameOnLogin();
		farmStatus();
		...
		getPackets().sendGameMessage("We hope you enjoy your stay.");
		...
		extremeDonatorTill = 0;
				if (getUsername().equalsIgnoreCase("Max")) {
				for (Player p : World.getPlayers()) {
				p.getPackets().sendGameMessage("<img=1><col=00FF73><shad=000000>Owner " + getUsername() + " has logged in.");
				}	
				} else if (getUsername().equalsIgnoreCase("Plasticboy22")) {
				for (Player p : World.getPlayers()) {
				p.getPackets().sendGameMessage("<img=1><col=C12006><shad=000000>Co-Owner " + getUsername() + " has logged in.");
				}					
		} else if (getRights() == 2) {
				for (Player p : World.getPlayers()) {
				p.getPackets().sendGameMessage("<img=1><col=FF0000><shad=000000>Admin " + getUsername() + " has logged in.");
				}
			} else if (getRights() == 1) {
				for (Player p : World.getPlayers()) {
				p.getPackets().sendGameMessage("<img=0><col=FF6600><shad=000000>Moderator " + getUsername() + " has logged in.");
	}			
		} else if (extremeDonator || extremeDonatorTill != 0) {
			...
			if (!superDonator && superDonatorTill < Utils.currentTimeMillis()) {
				getPackets().sendGameMessage("Your super donator rank expired.");
				superDonatorTill = 0;
			} else
				getPackets().sendGameMessage("Your super donator rank expires " + getSuperDonatorTill());
		...
		if (starter == 0) {
			for (Player p2: World.getPlayers()) {
				p2.getPackets().sendGameMessage("<col=01DFD7>Welcome<col> " + getDisplayName() + "<col=01DFD7> to Validus!</col>");
				FriendChatsManager.joinChat("max", p2);
	...
	public int getMessageIcon() {
		return getRights() == 2 || getRights() == 1 ? getRights() : isForumModerator() ? 10 : isGraphicDesigner() ? 9 : getRights() == 3 ? 10 : isExtremeDonator() ? 11 : isSuperDonator() ? 12 : isDonator() ? 8 : getRights();
	}
	...
	public int farmingStatusI = 0;
	public String farmStatusI = "Needs Raking.";
	
	public int farmingStatusJ = 0;
	public String farmStatusJ = "Needs Raking.";
	
	public int farmingStatusK = 0;
	public String farmStatusK = "Needs Raking.";
	
	public int farmingStatusL = 0;
	public String farmStatusL = "Needs Raking.";
	...
	public void farmStatus() {
		...
		switch (farmingStatusI) {
		case 0:
			farmStatusI = "Needs Raking.";
			break;
		case 1:
			farmStatusI = "This patch has been raked.";
			break;
		case 2:
			farmStatusI = "Growing.";
			break;
		case 3:
			farmStatusI = "Fully Grown, Ready to be harvested.";
			break;
		}
		switch (farmingStatusJ) {
		case 0:
			farmStatusJ = "Needs Raking.";
			break;
		case 1:
			farmStatusJ = "This patch has been raked.";
			break;
		case 2:
			farmStatusJ = "Growing.";
			break;
		case 3:
			farmStatusJ = "Fully Grown, Ready to be harvested.";
			break;
		}
		switch (farmingStatusK) {
		case 0:
			farmStatusK = "Needs Raking.";
			break;
		case 1:
			farmStatusK = "This patch has been raked.";
			break;
		case 2:
			farmStatusK = "Growing.";
			break;
		case 3:
			farmStatusK = "Fully Grown, Ready to be harvested.";
			break;
		}
		switch (farmingStatusL) {
		case 0:
			farmStatusL = "Needs Raking.";
			break;
		case 1:
			farmStatusL = "This patch has been raked.";
			break;
		case 2:
			farmStatusL = "Growing.";
			break;
		case 3:
			farmStatusL = "Fully Grown, Ready to be harvested..";
			break;
		}
	}
	...
	public boolean tomatoA = false;
	public boolean tomatoB = false;
	public boolean strawberryA = false;
	public boolean strawberryB = false;
	public boolean onionA = false;
	public boolean onionB = false;
	public boolean cabbageA = false;
	public boolean cabbageB = false;
	public boolean melonA = false;
	public boolean melonB = false;
	public boolean guamA = false;
	public boolean marrenA = false;
	public boolean tarromA = false;
	public boolean harraA = false;
	public boolean rannarA = false;
	public boolean toadA = false;
	public boolean iritA = false;
	public boolean wergA = false;
	public boolean avantA = false;
	public boolean kwuarmA = false;
	public boolean cadanA = false;
	public boolean lantA = false;
	public boolean dwarfA = false;
	public boolean fellA = false;
	public boolean goatA = false;
	public boolean snapA = false;
	public boolean torstol = false;
	public boolean gold = false;
	public boolean lily = false;
	public boolean rose = false;
	public boolean woad = false;
	public boolean limp = false;
	public boolean nast = false;
	...
	public boolean tomatoCA = false;
	public boolean tomatoCB = false;
	public boolean strawberryCA = false;
	public boolean strawberryCB = false;
	public boolean onionCA = false;
	public boolean onionCB = false;
	public boolean cabbageCA = false;
	public boolean cabbageCB = false;
	...
	public boolean marrenCA = false;
	public boolean tarromCA = false;
	public boolean harraCA = false;
	public boolean rannarCA = false;
	public boolean toadCA = false;
	public boolean iritCA = false;
	public boolean wergCA = false;
	public boolean avantCA = false;
	public boolean kwuarmCA = false;
	public boolean cadanCA = false;
	public boolean lantCA = false;
	public boolean dwarfCA = false;
	public boolean fellCA = false;
	public boolean goatCA = false;
	public boolean snapCA = false;
	public boolean torstolCA = false;
	public boolean roseC = false;
	public boolean nastC = false;
	public boolean goldC = false;
	public boolean woadC = false;
	public boolean limpC = false;
	...
	public boolean spokeToAgent = false;
		public boolean hasBoughttrap = false;
	/**
	 * 1 = Teak Frame
	 * 2 = Mahogany Frame
	 */
	...
	public int window = 0;
	public int icon = 0;
	public int musical = 0;	
	public int statue = 0;
	public int garden = 0;	
	public int lever = 0;
	public int trapdoor = 0;
	public int floor = 0;
	public int bench = 0;
	...
	public int table = 0;
	public int deco = 0;
	public int dresser = 0;	
	public int bed = 0;	
	public int wardrobe = 0;
	public int clock = 0;	
	public int clockmaking = 0;	
	public int workbench = 0;	
	public int repair = 0;
	public int heraldry = 0;
	public int tools = 0;	
	public int rug = 0;	
	public int rug1 = 0;
	public int rug2 = 0;
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 1) {
					...
					if (chair == 1) {
						World.spawnObject(new WorldObject(13581, 10, 2, boundChuncks[0] * 8 + 42, boundChuncks[1] * 8 + 36, 0), true);
						World.spawnObject(new WorldObject(13581, 10, 2, boundChuncks[0] * 8 + 44, boundChuncks[1] * 8 + 35, 0), true);
						World.spawnObject(new WorldObject(13581, 10, 2, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 35, 0), true);
						World.spawnObject(new WorldObject(13581, 10, 2, boundChuncks[0] * 8 + 45, boundChuncks[1] * 8 + 36, 0), true);
					} else if (chair == 2) {
						World.spawnObject(new WorldObject(13583, 10, 2, boundChuncks[0] * 8 + 42, boundChuncks[1] * 8 + 36, 0), true);
						World.spawnObject(new WorldObject(13583, 10, 2, boundChuncks[0] * 8 + 44, boundChuncks[1] * 8 + 35, 0), true);
						World.spawnObject(new WorldObject(13583, 10, 2, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 35, 0), true);
						World.spawnObject(new WorldObject(13583, 10, 2, boundChuncks[0] * 8 + 45, boundChuncks[1] * 8 + 36, 0), true);
					} else if (chair == 3) {
						World.spawnObject(new WorldObject(13584, 10, 2, boundChuncks[0] * 8 + 42, boundChuncks[1] * 8 + 36, 0), true);
						World.spawnObject(new WorldObject(13584, 10, 2, boundChuncks[0] * 8 + 44, boundChuncks[1] * 8 + 35, 0), true);
						World.spawnObject(new WorldObject(13584, 10, 2, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 35, 0), true);
						World.spawnObject(new WorldObject(13584, 10, 2, boundChuncks[0] * 8 + 45, boundChuncks[1] * 8 + 36, 0), true);
					} else if (chair == 4) {
						World.spawnObject(new WorldObject(13587, 10, 2, boundChuncks[0] * 8 + 42, boundChuncks[1] * 8 + 36, 0), true);
						World.spawnObject(new WorldObject(13587, 10, 2, boundChuncks[0] * 8 + 44, boundChuncks[1] * 8 + 35, 0), true);
						World.spawnObject(new WorldObject(13587, 10, 2, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 35, 0), true);						
						World.spawnObject(new WorldObject(13587, 10, 2, boundChuncks[0] * 8 + 45, boundChuncks[1] * 8 + 36, 0), true);
					}
					if (throne == 1) {
					...
					if (bookcase == 1) {
						...
						World.spawnObject(new WorldObject(13597, 10, 2, boundChuncks[0] * 8 + 47, boundChuncks[1] * 8 + 46, 0), true);
						...
						World.spawnObject(new WorldObject(13598, 10, 2, boundChuncks[0] * 8 + 47, boundChuncks[1] * 8 + 46, 0), true);
						...
						World.spawnObject(new WorldObject(13599, 10, 2, boundChuncks[0] * 8 + 47, boundChuncks[1] * 8 + 46, 0), true);		
					...
					if (firePlace == 1) {
						...
						World.spawnObject(new WorldObject(13610, 10, 2, boundChuncks[0] * 8 + 55, boundChuncks[1] * 8 + 44, 0), true);
						World.spawnObject(new WorldObject(13610, 10, 3, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 33, 0), true);						
					} else if (firePlace == 2) {
						World.spawnObject(new WorldObject(13612, 10, 1, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 39, 0), true);	
						World.spawnObject(new WorldObject(13612, 10, 2, boundChuncks[0] * 8 + 55, boundChuncks[1] * 8 + 44, 0), true);
						World.spawnObject(new WorldObject(13612, 10, 3, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 33, 0), true);						
					} else if (firePlace == 3) {
						World.spawnObject(new WorldObject(13614, 10, 1, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13614, 10, 2, boundChuncks[0] * 8 + 55, boundChuncks[1] * 8 + 44, 0), true);	
						World.spawnObject(new WorldObject(13614, 10, 3, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 33, 0), true);							
					} else if (firePlace == 4) {
						World.spawnObject(new WorldObject(13610, 10, 1, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 39, 0), true);	
					} else if (firePlace == 5) {
						World.spawnObject(new WorldObject(13612, 10, 1, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 39, 0), true);	
						...
						World.spawnObject(new WorldObject(13614, 10, 1, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 39, 0), true);	
						World.spawnObject(new WorldObject(13614, 10, 1, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 43, 0), true);							
					}
					switch (portalTele1) {
					case 1:
						portal1 = true;
					...
					switch (icon) {
					case 1:
						World.spawnObject(new WorldObject(13175, 10, 2, boundChuncks[0] * 8 + 35, boundChuncks[1] * 8 + 24, 0), true);
						break;
					case 2:
						World.spawnObject(new WorldObject(13176, 10, 2, boundChuncks[0] * 8 + 35, boundChuncks[1] * 8 + 24, 0), true);
						break;
					case 3:
						World.spawnObject(new WorldObject(13177, 10, 2, boundChuncks[0] * 8 + 35, boundChuncks[1] * 8 + 24, 0), true);
						break;
					}
					switch (musical) {
					case 1:
						World.spawnObject(new WorldObject(13214, 10, 1, boundChuncks[0] * 8 + 32, boundChuncks[1] * 8 + 27, 0), true);
						break;
					case 2:
						World.spawnObject(new WorldObject(13215, 10, 1, boundChuncks[0] * 8 + 32, boundChuncks[1] * 8 + 27, 0), true);
						break;
					case 3:
						World.spawnObject(new WorldObject(13216, 10, 3, boundChuncks[0] * 8 + 32, boundChuncks[1] * 8 + 27, 0), true);
						break;
					}
					switch (focus) {
					case 1:
						World.spawnObject(new WorldObject(13640, 10, 0, boundChuncks[0] * 8 + 35, boundChuncks[1] * 8 + 43, 0), true);
						break;
					case 2:
						World.spawnObject(new WorldObject(13641, 10, 0, boundChuncks[0] * 8 + 35, boundChuncks[1] * 8 + 43, 0), true);
						break;
					case 3:
						World.spawnObject(new WorldObject(13639, 10, 0, boundChuncks[0] * 8 + 35, boundChuncks[1] * 8 + 43, 0), true);
						break;
					}
					switch (heraldry) {
					case 1:
						World.spawnObject(new WorldObject(13716, 10, 0, boundChuncks[0] * 8 + 46, boundChuncks[1] * 8 + 24, 0), true);
						break;
					case 2:
						World.spawnObject(new WorldObject(13717, 10, 0, boundChuncks[0] * 8 + 46, boundChuncks[1] * 8 + 24, 0), true);
						break;
					case 3:
						World.spawnObject(new WorldObject(13718, 10, 0, boundChuncks[0] * 8 + 46, boundChuncks[1] * 8 + 24, 0), true);
						break;
					}					
					switch (bed) {
					case 1:
						World.spawnObject(new WorldObject(13150, 10, 1, boundChuncks[0] * 8 + 54, boundChuncks[1] * 8 + 36, 0), true);
						break;
					case 2:
						World.spawnObject(new WorldObject(13152, 10, 1, boundChuncks[0] * 8 + 54, boundChuncks[1] * 8 + 36, 0), true);
						break;
					case 3:
						World.spawnObject(new WorldObject(13154, 10, 1, boundChuncks[0] * 8 + 54, boundChuncks[1] * 8 + 36, 0), true);
						break;
					}	
					switch (wardrobe) {
					case 1:
						World.spawnObject(new WorldObject(13157, 10, 1, boundChuncks[0] * 8 + 55, boundChuncks[1] * 8 + 33, 0), true);
						break;
					case 2:
						World.spawnObject(new WorldObject(13159, 10, 1, boundChuncks[0] * 8 + 55, boundChuncks[1] * 8 + 33, 0), true);
						break;
					case 3:
						World.spawnObject(new WorldObject(13161, 10, 1, boundChuncks[0] * 8 + 55, boundChuncks[1] * 8 + 33, 0), true);
						break;
					}
					switch (clock) {
					case 1:
						World.spawnObject(new WorldObject(13169, 10, 3, boundChuncks[0] * 8 + 48, boundChuncks[1] * 8 + 32, 0), true);
						break;
					case 2:
						World.spawnObject(new WorldObject(13170, 10, 3, boundChuncks[0] * 8 + 48, boundChuncks[1] * 8 + 32, 0), true);
						break;
					case 3:
						World.spawnObject(new WorldObject(13171, 10, 3, boundChuncks[0] * 8 + 48, boundChuncks[1] * 8 + 32, 0), true);
						break;
					}	
					switch (clockmaking) {
					case 1:
						World.spawnObject(new WorldObject(13710, 10, 0, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 31, 0), true);
						break;
					case 2:
						World.spawnObject(new WorldObject(13711, 10, 0, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 31, 0), true);
						break;
					case 3:
						World.spawnObject(new WorldObject(13712, 10, 0, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 31, 0), true);
						break;
					}					
					switch (bell) {
					case 1:
						World.spawnObject(new WorldObject(13307, 10, 3, boundChuncks[0] * 8 + 48, boundChuncks[1] * 8 + 47, 0), true);
						break;
					case 2:
						World.spawnObject(new WorldObject(13308, 10, 3, boundChuncks[0] * 8 + 48, boundChuncks[1] * 8 + 47, 0), true);
						break;
					case 3:
						World.spawnObject(new WorldObject(13309, 10, 3, boundChuncks[0] * 8 + 48, boundChuncks[1] * 8 + 47, 0), true);
						break;
					}	
					switch (workbench) {
					case 1:
						World.spawnObject(new WorldObject(13706, 10, 1, boundChuncks[0] * 8 + 44, boundChuncks[1] * 8 + 27, 0), true);
						break;
					case 2:
						World.spawnObject(new WorldObject(13707, 10, 1, boundChuncks[0] * 8 + 4, boundChuncks[1] * 8 + 27, 0), true);
						break;
					case 3:
						World.spawnObject(new WorldObject(13708, 10, 1, boundChuncks[0] * 8 + 44, boundChuncks[1] * 8 + 27, 0), true);
						break;
					}						
					switch (stair) {
					case 1:
						World.spawnObject(new WorldObject(13499, 10, 0, boundChuncks[0] * 8 + 44, boundChuncks[1] * 8 + 43, 0), true);
						break;
					case 2:
						World.spawnObject(new WorldObject(13501, 10, 0, boundChuncks[0] * 8 + 44, boundChuncks[1] * 8 + 43, 0), true);
						break;
					}					
					switch (statue) {
					case 1:
						World.spawnObject(new WorldObject(13271, 10, 0, boundChuncks[0] * 8 + 32, boundChuncks[1] * 8 + 31, 0), true);
						World.spawnObject(new WorldObject(13271, 10, 0, boundChuncks[0] * 8 + 39, boundChuncks[1] * 8 + 31, 0), true);						
						break;
					case 2:
						World.spawnObject(new WorldObject(13274, 10, 0, boundChuncks[0] * 8 + 32, boundChuncks[1] * 8 + 31, 0), true);
						World.spawnObject(new WorldObject(13274, 10, 0, boundChuncks[0] * 8 + 39, boundChuncks[1] * 8 + 31, 0), true);			
						break;
					case 3:
						World.spawnObject(new WorldObject(13277, 10, 0, boundChuncks[0] * 8 + 32, boundChuncks[1] * 8 + 31, 0), true);
						World.spawnObject(new WorldObject(13277, 10, 0, boundChuncks[0] * 8 + 39, boundChuncks[1] * 8 + 31, 0), true);						
						break;
					}						
					switch (lamp) {
					case 1:
						World.spawnObject(new WorldObject(13209, 10, 0, boundChuncks[0] * 8 + 33, boundChuncks[1] * 8 + 26, 0), true);
						World.spawnObject(new WorldObject(13209, 10, 0, boundChuncks[0] * 8 + 38, boundChuncks[1] * 8 + 26, 0), true);						
						break;
					case 2:
						World.spawnObject(new WorldObject(13211, 10, 0, boundChuncks[0] * 8 + 33, boundChuncks[1] * 8 + 26, 0), true);
					World.spawnObject(new WorldObject(13211, 10, 0, boundChuncks[0] * 8 + 38, boundChuncks[1] * 8 + 26, 0), true);
						break;
					case 3:
						World.spawnObject(new WorldObject(52824, 10, 0, boundChuncks[0] * 8 + 33, boundChuncks[1] * 8 + 26, 0), true);		
		World.spawnObject(new WorldObject(52824, 10, 0, boundChuncks[0] * 8 + 38, boundChuncks[1] * 8 + 26, 0), true);
						break;
					}
					switch (table) {
					case 1:
						World.spawnObject(new WorldObject(13297, 10, 1, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 43, 0), true);
						
						break;
					case 2:
						World.spawnObject(new WorldObject(13298, 10, 1, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 43, 0), true);
				
						break;
					case 3:
						World.spawnObject(new WorldObject(13299, 10, 1, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 43, 0), true);		
	
						break;
					}
					switch (ladder) {
					case 1:
						World.spawnObject(new WorldObject(13328, 10, 1, boundChuncks[0] * 8 + 54, boundChuncks[1] * 8 + 30, 0), true);
						
						break;
					case 2:
						World.spawnObject(new WorldObject(13329, 10, 1, boundChuncks[0] * 8 + 54, boundChuncks[1] * 8 + 30, 0), true);
				
						break;
					case 3:
						World.spawnObject(new WorldObject(13330, 10, 1, boundChuncks[0] * 8 + 54, boundChuncks[1] * 8 + 30, 0), true);		
	
						break;
					}					
					switch (dresser) {
					case 1:
						World.spawnObject(new WorldObject(13163, 10, 1, boundChuncks[0] * 8 + 55, boundChuncks[1] * 8 + 38, 0), true);
						
						break;
					case 2:
						World.spawnObject(new WorldObject(13164, 10, 1, boundChuncks[0] * 8 + 55, boundChuncks[1] * 8 + 38, 0), true);
				
						break;
					case 3:
						World.spawnObject(new WorldObject(13168, 10, 1, boundChuncks[0] * 8 + 55, boundChuncks[1] * 8 + 38, 0), true);		
	
						break;
					}					
					switch (bench) {
					case 1:
						World.spawnObject(new WorldObject(13304, 10, 0, boundChuncks[0] * 8 + 26, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13304, 10, 0, boundChuncks[0] * 8 + 27, boundChuncks[1] * 8 + 39, 0), true);	
							World.spawnObject(new WorldObject(13304, 10, 0, boundChuncks[0] * 8 + 28, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13304, 10, 0, boundChuncks[0] * 8 + 29, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13304, 10, 0, boundChuncks[0] * 8 + 30, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13304, 10, 0, boundChuncks[0] * 8 + 31, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13304, 10, 2, boundChuncks[0] * 8 + 26, boundChuncks[1] * 8 + 32, 0), true);
						World.spawnObject(new WorldObject(13304, 10, 2, boundChuncks[0] * 8 + 27, boundChuncks[1] * 8 + 32, 0), true);	
							World.spawnObject(new WorldObject(13304, 10, 2, boundChuncks[0] * 8 + 28, boundChuncks[1] * 8 + 32, 0), true);
						World.spawnObject(new WorldObject(13304, 10, 2, boundChuncks[0] * 8 + 29, boundChuncks[1] * 8 + 32, 0), true);
						World.spawnObject(new WorldObject(13304, 10, 2, boundChuncks[0] * 8 + 30, boundChuncks[1] * 8 + 32, 0), true);
						World.spawnObject(new WorldObject(13304, 10, 2, boundChuncks[0] * 8 + 31, boundChuncks[1] * 8 + 32, 0), true);						
						break;
					case 2:
						World.spawnObject(new WorldObject(13305, 10, 0, boundChuncks[0] * 8 + 26, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13305, 10, 0, boundChuncks[0] * 8 + 27, boundChuncks[1] * 8 + 39, 0), true);	
							World.spawnObject(new WorldObject(13305, 10, 0, boundChuncks[0] * 8 + 28, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13305, 10, 0, boundChuncks[0] * 8 + 29, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13305, 10, 0, boundChuncks[0] * 8 + 30, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13305, 10, 0, boundChuncks[0] * 8 + 31, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13305, 10, 2, boundChuncks[0] * 8 + 26, boundChuncks[1] * 8 + 32, 0), true);
						World.spawnObject(new WorldObject(13305, 10, 2, boundChuncks[0] * 8 + 27, boundChuncks[1] * 8 + 32, 0), true);	
							World.spawnObject(new WorldObject(13305, 10, 2, boundChuncks[0] * 8 + 28, boundChuncks[1] * 8 + 32, 0), true);
						World.spawnObject(new WorldObject(13305, 10, 2, boundChuncks[0] * 8 + 29, boundChuncks[1] * 8 + 32, 0), true);
						World.spawnObject(new WorldObject(13305, 10, 2, boundChuncks[0] * 8 + 30, boundChuncks[1] * 8 + 32, 0), true);
						World.spawnObject(new WorldObject(13305, 10, 2, boundChuncks[0] * 8 + 31, boundChuncks[1] * 8 + 32, 0), true);						
						break;
					case 3:
						World.spawnObject(new WorldObject(13306, 10, 0, boundChuncks[0] * 8 + 26, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13306, 10, 0, boundChuncks[0] * 8 + 27, boundChuncks[1] * 8 + 39, 0), true);	
							World.spawnObject(new WorldObject(13306, 10, 0, boundChuncks[0] * 8 + 28, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13306, 10, 0, boundChuncks[0] * 8 + 29, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13306, 10, 0, boundChuncks[0] * 8 + 30, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13306, 10, 0, boundChuncks[0] * 8 + 31, boundChuncks[1] * 8 + 39, 0), true);
						World.spawnObject(new WorldObject(13306, 10, 2, boundChuncks[0] * 8 + 26, boundChuncks[1] * 8 + 32, 0), true);
						World.spawnObject(new WorldObject(13306, 10, 2, boundChuncks[0] * 8 + 27, boundChuncks[1] * 8 + 32, 0), true);	
							World.spawnObject(new WorldObject(13306, 10, 2, boundChuncks[0] * 8 + 28, boundChuncks[1] * 8 + 32, 0), true);
						World.spawnObject(new WorldObject(13306, 10, 2, boundChuncks[0] * 8 + 29, boundChuncks[1] * 8 + 32, 0), true);
						World.spawnObject(new WorldObject(13306, 10, 2, boundChuncks[0] * 8 + 30, boundChuncks[1] * 8 + 32, 0), true);
						World.spawnObject(new WorldObject(13306, 10, 2, boundChuncks[0] * 8 + 31, boundChuncks[1] * 8 + 32, 0), true);							
						break;
					}
					switch (seat) {
					case 1:
						World.spawnObject(new WorldObject(13304, 10, 3, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 45, 0), true);
						World.spawnObject(new WorldObject(13304, 10, 3, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 44, 0), true);	
						World.spawnObject(new WorldObject(13304, 10, 3, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 43, 0), true);
						World.spawnObject(new WorldObject(13304, 10, 3, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 42, 0), true);
						World.spawnObject(new WorldObject(13304, 10, 1, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 45, 0), true);
						World.spawnObject(new WorldObject(13304, 10, 1, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 44, 0), true);
						World.spawnObject(new WorldObject(13304, 10, 1, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 43, 0), true);
						World.spawnObject(new WorldObject(13304, 10, 1, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 42, 0), true);	
						
						break;
					case 2:
						World.spawnObject(new WorldObject(13305, 10, 3, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 45, 0), true);
						World.spawnObject(new WorldObject(13305, 10, 3, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 44, 0), true);	
						World.spawnObject(new WorldObject(13305, 10, 3, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 43, 0), true);
						World.spawnObject(new WorldObject(13305, 10, 3, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 42, 0), true);
						World.spawnObject(new WorldObject(13305, 10, 1, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 45, 0), true);
						World.spawnObject(new WorldObject(13305, 10, 1, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 44, 0), true);
						World.spawnObject(new WorldObject(13305, 10, 1, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 43, 0), true);
						World.spawnObject(new WorldObject(13305, 10, 1, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 42, 0), true);						
						break;
					case 3:
						World.spawnObject(new WorldObject(13306, 10, 3, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 45, 0), true);
						World.spawnObject(new WorldObject(13306, 10, 3, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 44, 0), true);	
						World.spawnObject(new WorldObject(13306, 10, 3, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 43, 0), true);
						World.spawnObject(new WorldObject(13306, 10, 3, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 42, 0), true);
						World.spawnObject(new WorldObject(13306, 10, 1, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 45, 0), true);
						World.spawnObject(new WorldObject(13306, 10, 1, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 44, 0), true);
						World.spawnObject(new WorldObject(13306, 10, 1, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 43, 0), true);
						World.spawnObject(new WorldObject(13306, 10, 1, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 42, 0), true);							
						break;
					}					
					switch (garden) {
					case 1:
						World.spawnObject(new WorldObject(13427, 10, 0, boundChuncks[0] * 8 + 38, boundChuncks[1] * 8 + 32, 0), true);
						World.spawnObject(new WorldObject(13434, 10, 0, boundChuncks[0] * 8 + 35, boundChuncks[1] * 8 + 33, 0), true);
						World.spawnObject(new WorldObject(13434, 10, 0, boundChuncks[0] * 8 + 36, boundChuncks[1] * 8 + 37, 0), true);
						World.spawnObject(new WorldObject(13430, 10, 0, boundChuncks[0] * 8 + 32, boundChuncks[1] * 8 + 32, 0), true);						
						break;

					}
					switch (firefloor) {
					case 1:
						World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 29, 0), true);
						World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 28, 0), true);
						World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 27, 0), true);
						World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 26, 0), true);	
							World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 52, boundChuncks[1] * 8 + 29, 0), true);
						World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 52, boundChuncks[1] * 8 + 28, 0), true);
						World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 52, boundChuncks[1] * 8 + 27, 0), true);
						World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 52, boundChuncks[1] * 8 + 26, 0), true);	
						World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 29, 0), true);
						World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 28, 0), true);
						World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 27, 0), true);
						World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 26, 0), true);
						World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 29, 0), true);
						World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 28, 0), true);
						World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 27, 0), true);
						World.spawnObject(new WorldObject(25156, 10, 0, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 26, 0), true);							
						break;

					}					
					switch (sword) {
					case 1:
						World.spawnObject(new WorldObject(12237, 10, 1, boundChuncks[0] * 8 + 47, boundChuncks[1] * 8 + 41, 0), true);
						World.spawnObject(new WorldObject(56056, 10, 2, boundChuncks[0] * 8 + 44, boundChuncks[1] * 8 + 42, 0), true);
						World.spawnObject(new WorldObject(24111, 10, 0, boundChuncks[0] * 8 + 40, boundChuncks[1] * 8 + 41, 0), true);
						World.spawnObject(new WorldObject(24109, 10, 2, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 40, 0), true);
						World.spawnObject(new WorldObject(24110, 10, 0, boundChuncks[0] * 8 + 40, boundChuncks[1] * 8 + 46, 0), true);						
						break;

					}
						switch (tools) {
					case 1:
						World.spawnObject(new WorldObject(24804, 10, 0, boundChuncks[0] * 8 + 40, boundChuncks[1] * 8 + 25, 0), true);
						World.spawnObject(new WorldObject(54608, 10, 1, boundChuncks[0] * 8 + 41, boundChuncks[1] * 8 + 24, 0), true);
						World.spawnObject(new WorldObject(9662, 10, 0, boundChuncks[0] * 8 + 40, boundChuncks[1] * 8 + 30, 0), true);
						World.spawnObject(new WorldObject(15468, 10, 2, boundChuncks[0] * 8 + 41, boundChuncks[1] * 8 + 31, 0), true);
						World.spawnObject(new WorldObject(46317, 10, 0, boundChuncks[0] * 8 + 46, boundChuncks[1] * 8 + 31, 0), true);						
						break;

					}				
						switch (decoration) {
					case 1:
						World.spawnObject(new WorldObject(15562, 10, 4, boundChuncks[0] * 8 + 24, boundChuncks[1] * 8 + 36, 0), true);
						World.spawnObject(new WorldObject(15562, 10, 4, boundChuncks[0] * 8 + 24, boundChuncks[1] * 8 + 35, 0), true);
						...
						break;
							...
							switch (deco) {
					case 1:
						World.spawnObject(new WorldObject(13490, 10, 1, boundChuncks[0] * 8 + 55, boundChuncks[1] * 8 + 42, 0), true);
						World.spawnObject(new WorldObject(13488, 10, 1, boundChuncks[0] * 8 + 55, boundChuncks[1] * 8 + 45, 0), true);
					...
					switch (lever) {
					case 1:
						World.spawnObject(new WorldObject(13674, 10, 1, boundChuncks[0] * 8 + 25, boundChuncks[1] * 8 + 38, 0), true);
					
						break;
						}
					switch (cage) {
					case 1:
						World.spawnObject(new WorldObject(20692, 10, 2, boundChuncks[0] * 8 + 49, boundChuncks[1] * 8 + 28, 0), true);
						World.spawnObject(new WorldObject(20692, 10, 2, boundChuncks[0] * 8 + 49, boundChuncks[1] * 8 + 29, 0), true);						
						World.spawnObject(new WorldObject(20692, 10, 1, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 27, 0), true);
						World.spawnObject(new WorldObject(20692, 10, 3, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 32, 0), true);
						World.spawnObject(new WorldObject(20692, 10, 3, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 32, 0), true);						
						World.spawnObject(new WorldObject(20692, 10, 0, boundChuncks[0] * 8 + 54, boundChuncks[1] * 8 + 28, 0), true);	
						World.spawnObject(new WorldObject(20692, 10, 0, boundChuncks[0] * 8 + 54, boundChuncks[1] * 8 + 29, 0), true);	
						World.spawnObject(new WorldObject(65366, 10, 2, boundChuncks[0] * 8 + 53, boundChuncks[1] * 8 + 25, 0), true);							
					
						break;
						}						
						switch (guard) {
					case 1:
						World.spawnObject(new WorldObject(2715, 10, 0, boundChuncks[0] * 8 + 48, boundChuncks[1] * 8 + 30, 0), true);
					...
					switch (repair) {
					case 1:
						World.spawnObject(new WorldObject(13713, 10, 0, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 24, 0), true);
					...
					switch (trapdoor) {
					case 1:
						World.spawnObject(new WorldObject(105, 10, 2, boundChuncks[0] * 8 + 25, boundChuncks[1] * 8 + 33, 0), true);
						...
						switch (floor) {
					case 1:
						World.spawnObject(new WorldObject(14051, 10, 0, boundChuncks[0] * 8 + 27, boundChuncks[1] * 8 + 35, 0), true);
						World.spawnObject(new WorldObject(14051, 10, 0, boundChuncks[0] * 8 + 28, boundChuncks[1] * 8 + 35, 0), true);
						World.spawnObject(new WorldObject(14051, 10, 0, boundChuncks[0] * 8 + 27, boundChuncks[1] * 8 + 36, 0), true);
						World.spawnObject(new WorldObject(14051, 10, 0, boundChuncks[0] * 8 + 28, boundChuncks[1] * 8 + 36, 0), true);						
					...
					switch (window) {
					case 1:
						World.spawnObject(new WorldObject(24766, 10, 1, boundChuncks[0] * 8 + 34, boundChuncks[1] * 8 + 23, 0), true);
						World.spawnObject(new WorldObject(24766, 10, 1, boundChuncks[0] * 8 + 37, boundChuncks[1] * 8 + 23, 0), true);
						World.spawnObject(new WorldObject(24766, 10, 0, boundChuncks[0] * 8 + 32, boundChuncks[1] * 8 + 26, 0), true);
						World.spawnObject(new WorldObject(24766, 10, 0, boundChuncks[0] * 8 + 32, boundChuncks[1] * 8 + 29, 0), true);						
						break;

					}
					switch (rug) {
					case 1:
						World.spawnObject(new WorldObject(3582 , 10, 0, boundChuncks[0] * 8 + 36, boundChuncks[1] * 8 + 28, 0), true);
						World.spawnObject(new WorldObject(3582 , 10, 0, boundChuncks[0] * 8 + 35, boundChuncks[1] * 8 + 28, 0), true);	
						World.spawnObject(new WorldObject(3582 , 10, 0, boundChuncks[0] * 8 + 36, boundChuncks[1] * 8 + 29, 0), true);
						World.spawnObject(new WorldObject(3582 , 10, 0, boundChuncks[0] * 8 + 35, boundChuncks[1] * 8 + 29, 0), true);	
						World.spawnObject(new WorldObject(3582 , 10, 0, boundChuncks[0] * 8 + 36, boundChuncks[1] * 8 + 30, 0), true);
						World.spawnObject(new WorldObject(3582 , 10, 0, boundChuncks[0] * 8 + 35, boundChuncks[1] * 8 + 30, 0), true);						
						break;

					}
					switch (rug1) {
					case 1:
						World.spawnObject(new WorldObject(7394 , 10, 0, boundChuncks[0] * 8 + 42, boundChuncks[1] * 8 + 35, 0), true);
						World.spawnObject(new WorldObject(7394 , 10, 0, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 34, 0), true);	
						World.spawnObject(new WorldObject(7394 , 10, 0, boundChuncks[0] * 8 + 44, boundChuncks[1] * 8 + 34, 0), true);	
						World.spawnObject(new WorldObject(7394 , 10, 0, boundChuncks[0] * 8 + 45, boundChuncks[1] * 8 + 35, 0), true);	
					...
					switch (rug2) {
					case 1:
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 49, boundChuncks[1] * 8 + 33, 0), true);
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 49, boundChuncks[1] * 8 + 34, 0), true);	
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 49, boundChuncks[1] * 8 + 35, 0), true);	
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 49, boundChuncks[1] * 8 + 36, 0), true);	
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 49, boundChuncks[1] * 8 + 37, 0), true);
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 49, boundChuncks[1] * 8 + 38, 0), true);	
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 38, 0), true);	
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 38, 0), true);	
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 52, boundChuncks[1] * 8 + 38, 0), true);
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 52, boundChuncks[1] * 8 + 37, 0), true);	
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 52, boundChuncks[1] * 8 + 36, 0), true);	
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 52, boundChuncks[1] * 8 + 35, 0), true);
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 52, boundChuncks[1] * 8 + 34, 0), true);
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 52, boundChuncks[1] * 8 + 33, 0), true);	
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 33, 0), true);	
						World.spawnObject(new WorldObject(63489 , 10, 0, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 33, 0), true);	
							World.spawnObject(new WorldObject(63488 , 10, 0, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 34, 0), true);
						World.spawnObject(new WorldObject(63488 , 10, 0, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 35, 0), true);	
						World.spawnObject(new WorldObject(63488 , 10, 0, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 36, 0), true);	
						World.spawnObject(new WorldObject(63488 , 10, 0, boundChuncks[0] * 8 + 50, boundChuncks[1] * 8 + 37, 0), true);
						World.spawnObject(new WorldObject(63488 , 10, 0, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 34, 0), true);
						World.spawnObject(new WorldObject(63488 , 10, 0, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 35, 0), true);	
						World.spawnObject(new WorldObject(63488 , 10, 0, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 36, 0), true);	
						World.spawnObject(new WorldObject(63488 , 10, 0, boundChuncks[0] * 8 + 51, boundChuncks[1] * 8 + 37, 0), true);						
					...
					switch (rug3) {
					case 1:
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 41, boundChuncks[1] * 8 + 41, 0), true);
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 42, boundChuncks[1] * 8 + 41, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 41, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 44, boundChuncks[1] * 8 + 41, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 45, boundChuncks[1] * 8 + 41, 0), true);
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 46, boundChuncks[1] * 8 + 41, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 41, boundChuncks[1] * 8 + 42, 0), true);
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 42, boundChuncks[1] * 8 + 42, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 42, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 44, boundChuncks[1] * 8 + 42, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 45, boundChuncks[1] * 8 + 42, 0), true);
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 46, boundChuncks[1] * 8 + 42, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 41, boundChuncks[1] * 8 + 43, 0), true);
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 42, boundChuncks[1] * 8 + 43, 0), true);
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 45, boundChuncks[1] * 8 + 43, 0), true);
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 46, boundChuncks[1] * 8 + 43, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 41, boundChuncks[1] * 8 + 44, 0), true);
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 42, boundChuncks[1] * 8 + 44, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 44, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 44, boundChuncks[1] * 8 + 44, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 45, boundChuncks[1] * 8 + 44, 0), true);
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 46, boundChuncks[1] * 8 + 44, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 41, boundChuncks[1] * 8 + 45, 0), true);
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 42, boundChuncks[1] * 8 + 45, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 45, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 44, boundChuncks[1] * 8 + 45, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 45, boundChuncks[1] * 8 + 45, 0), true);
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 46, boundChuncks[1] * 8 + 45, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 41, boundChuncks[1] * 8 + 46, 0), true);
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 42, boundChuncks[1] * 8 + 46, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 43, boundChuncks[1] * 8 + 46, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 44, boundChuncks[1] * 8 + 46, 0), true);	
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 45, boundChuncks[1] * 8 + 46, 0), true);
						World.spawnObject(new WorldObject(12874 , 10, 0, boundChuncks[0] * 8 + 46, boundChuncks[1] * 8 + 46, 0), true);							
						break;
					}
	...
	public void setCompletedPrestigeOne() {
		...
		if (prestigeNumber == 5) {
			getPackets().sendGameMessage("<col=ff0000>You have reached the final prestige level, you can no longer prestige.");
			}
		}
	}
	...
	public void skillCape() {
		if (fish >= 749 && logs >= 249 && wc >= 349 && slayer >= 699 && thieve >= 699 && agility >= 99 && ore >= 999 && barbLap >= 89 && cook >= 449
	...
	public void completedCompletionistCape() {
		for (int i = 0; i < 23; i++) {
			if (this.getSkills().getLevel(i) >= 99 && getSkills().getLevel(24) >= 120 && fish >= 749 && wc >= 349
					 && slayer >= 699 && thieve >= 699 && agility >= 99) {
				setCompletedComp();
				return;
			} else {
			...
			public void run() {
				if (loop == 0) {
					...
					CoresManager.slowExecutor.execute(new Runnable() {
						...
						public void run() {
							reset();
						setNextWorldTile(new WorldTile(3684, 2954, 0));
							setNextAnimation(new Animation(-1));
							WorldTasksManager.schedule(new WorldTask()  {
								@Override
								public void run() {
	...
	public void lock() {
		...
		if (Settings.DEBUG) {
			Logger.log(this, new StringBuilder("Lobby Inited Player: ").append(string).append(", pass: ").append(Encrypt.encryptSHA1(password).toString()));
		}
	}
	...
	private boolean profranityFilter;

	public void switchProfanityFilter() {
		profranityFilter = !profranityFilter;
		refreshProfanityFilter();
	}
	  
    public void refreshProfanityFilter() {
    	getVarBitManager().sendVarBit(8780, profranityFilter ? 0 : 1);
    }
	...
	public String getSuperDonatorTill() {
		return (superDonator ? "never" : new Date(superDonatorTill).toGMTString()) + ".";
	}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (count++ == 1) {
					...
					if (Utils.random(0) > 1)
						getPackets().sendGameMessage("You manage to get some need oil");
