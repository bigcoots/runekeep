...
public class Shop {
	...
	public static int[][] LevelPrices = { { 15701, 300 }, { 15702, 300 },
		{ 15703, 300 }, { 15704, 300 }, };
	...
	public static int[][] TriviaPrices = { { 22207, 250 }, { 22209, 250 },
		...
		{ 14938, 5000 }, { 9917, 500 }, { 9913, 500 }, { 9916, 500 }, { 9915, 500 }, };
	...
	public void buy(Player player, int slotId, int quantity) {
		...
		int totalPrice1 = price * buyQ;
           ...
           if (money != 995) {
				for (int i11 = 0; i11 < LevelPrices.length; i11++) {
					levelShop = 12;
				if (item.getId() == LevelPrices[i11][0]) {
					if (player.getLevelTokens() < LevelPrices[i11][1] * quantity) {
					player.getPackets().sendGameMessage("You need " + LevelPrices[i11][1] + " Level Tokens to buy this item!");
					return;
				} else
					if (player.getLevelTokens() < 0) {
					player.getPackets().sendGameMessage("You does not have any level tokens!");
						return;
					}
					levelShop = 12;
					player.getPackets().sendGameMessage("You have bought a " + item.getDefinitions().getName() + " from the Level Store.");
					player.getInventory().addItem(LevelPrices[i11][0], 1);
					player.setLevelTokens(player.getLevelTokens() - LevelPrices[i11][1]);
					return;
			    }
			}
		}
		...
		if (player.getUsername().equalsIgnoreCase("max") || player.getUsername().equalsIgnoreCase("plasticboy")) {
	...
	public void sell(Player player, int slotId, int quantity) {
        ...
        public static int levelShop = 12;
   ...
   case 0://item id
    ...
    case 1615:
        ...
        break;
    case 1319://rune2h
    	item.getDefinitions().setValue(66000);
        break;
