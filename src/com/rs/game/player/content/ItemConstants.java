...
public class ItemConstants {
	...
	public static boolean isTradeable(Item item) {
		if (item.getDefinitions().isLended()
			...
			return false;
		if (item.getDefinitions().getName().toLowerCase().contains("flaming skull") || item.getDefinitions().getName().toLowerCase().contains("drygore rapier") || item.getDefinitions().getName().toLowerCase().contains("drygore longsword") || item.getDefinitions().getName().toLowerCase().contains("drygore mace") || item.getDefinitions().getName().toLowerCase().contains("drygore rapier") || item.getDefinitions().getName().toLowerCase().contains("drygore"))
			return true;
		switch (item.getId()) {
		case 6570: //firecape
		case 29999: //donar cape
		case 6529: //tokkul
