...
public class DwarfCannon implements Serializable {
	...
	public void lostCannon() {
		if (player.hasSetupCannon == true) {
			if (player.getInventory().getFreeSlots() > 4) {
				...
				player.getBank().addItem(12, 1, true);
				player.hasSetupCannon = false;
				player.out("<col=1B9EE0>Your cannon has been placed in your Bank.");
				}
			}
	}
	...
	public void cannonSetup() {
	    	...
	    	if (World.isCannonArea(getPlayer())) {
    		player.getPackets().sendGameMessage("You cannot setup a cannon here");
    		return;
    	}
		if (hasCannon()) {
	...
	public void pickUpDwarfCannon(int stage, WorldObject object) {
		...
		if (stage == 0) {
			...
			getPlayer().getInventory().addItem(ITEMS[3], 1);*/
                        player.getBank().addItem(ITEMS[0], 1, true);
                        player.getBank().addItem(ITEMS[1], 1, true);
                        player.getBank().addItem(ITEMS[2], 1, true);
                        player.getBank().addItem(ITEMS[3], 1, true);
			if (getCannonBalls() < 1) {

			} else {
				player.getBank().addItem(2, cannonBalls, true);
			}
			getPlayer().setNextAnimation(new Animation(827));
			setCannonBalls(0);
			setRotating(false);
