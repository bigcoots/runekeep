...
public class PlayerCombat extends Action {
				...
				if (weaponName.contains("dragon dart")) {
                        if (target instanceof Player) {
							...
							if (weaponName.contains("deathtouched dart")) {
			    					...
			    					getTarget().addReceivedDamage(player, n.getHitpoints());
                            					n.applyHit(new Hit(n, n.getHitpoints(), HitLook.RANGE_DAMAGE, 0));  				
                        					}
		    					}					
				} else if (weaponName.contains("crossbow")) {
					int damage = 0;
					...
					if (weaponId == 11235 || weaponId == 15701
						...
						if (weaponId != -1) {
							if (!weaponName.endsWith("bow full")
									&& !weaponName.equals("zaryte bow")&& !weaponName.equals("decimation")&& !weaponName.equals("boogie bow"))
								dropAmmo(player);
						}
					}
				}
	...
	public int getArrowProjectileGfxId(int weaponId, int arrowId) {
		if (arrowId == 884) {
			...
			return 3196;
		else if (weaponId == 24456)
			return 336;			
		else if (weaponId == 16887)
			return 1066;
		else if (weaponId == 16337)
			return 1066;			
...
case 24455:// Annihilation
				player.setNextForceTalk(new ForceTalk("Die!"));
		...
		case 23695:
			...
			case 3204: // d hally
				...
				if (target.getSize() < 3) {// giant npcs wont get stunned cuz of
					// a stupid hit
...
case 24455: // Annihilation		
	...
	private final int getMaxHit(Player player, int weaponId,
		...
		if (!ranging) {
			...
			if (fullbalmungEquipped(player)) {
				double hp = player.getHitpoints();
				double maxhp = player.getMaxHitpoints();
				double d = hp / maxhp;
				otherBonus = 2.5 - d;

					player.setNextGraphics(new Graphics(2273, 0, 0));			
	...
	private double getSpecialAccuracyModifier(Player player) {
		...
		if (name.contains("anchor") || name.contains("magic longbow"))
			return 2;
		if (name.contains("dragon claws") || name.contains("armadyl godsword") || name.contains("annihilation"))
			return 2;
		return 1;
	}
				...
				if (player.getEquipment().getItem((Equipment.SLOT_HANDS)) != null) {
					if (player.getEquipment().getItem(Equipment.SLOT_HANDS)
						...
						if (name.contains("defender")
								...
								&& name.contains("frostmourne offhand"))
							return true;
						return true;
					}
				}
	...
	public static final boolean fullbalmungEquipped(Player player) {
		int weaponId = player.getEquipment().getWeaponId();
		if (weaponId == -1)
			return false;
		return 
		...
		if (weaponId != -1) {
			...
			if (weaponName != null && !weaponName.equals("null")) {
				...
				if (weaponName.contains("chinchompa")) {//TODO
					...
					return 16959;	
				if (weaponName.contains("boogie bow"))					
					return 426;					
	...
	if (weaponName.contains("annihilation")) {
				...
				if (weaponName.contains("balmung")) {
					switch (attackStyle) {
					case 2:
