	...
	public void start() {
		...
		sendEntityDialogue(SEND_2_TEXT_CHAT,
				new String[] { NPCDefinitions.getNPCDefinitions(npcId).name,
						"Hello! I can teleport you to several monsters and misc locations,",
		...
		if (stage == -1) {
			...
			if (componentId == OPTION_1)
				...
				sendOptionsDialogue("Where would you like to go?",
						"Revenants (Dangerous)", "Ganodermic Beasts", "Glacors",
			...
			if (componentId == OPTION_1) {
				...
				stage = 5;
				sendOptionsDialogue("Where would you like to go?",
						"Easts (PvP)", "BrimHaven", "Corporeal beast", "Dagannoth's",
						"More Options");
			}
		} else if (stage == 5) {
			if (componentId == OPTION_1) {
				...
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(2914,
						4448, 0));
			} else if (componentId == OPTION_5) {
				stage = 6;
				sendOptionsDialogue("Where would you like to go?",
			...
			/*else if (componentId == 2)
				...
				sendOptionsDialogue("Where would you like to go?",
						"Kbd", "Sunfreet", "Pest queen",
						"Blink", "Back to the begin.");
				stage = 8;
			...
			Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(3672, 6045, 2));
			else if (componentId == OPTION_3)
			Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(4520, 5538, 0));
			else if (componentId == OPTION_4)
			Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(2708, 9564, 0));			
			else if (componentId == OPTION_5) {
			sendOptionsDialogue("Where would you like to go?", "Nex.",
					"Bandos", "Armadyl", "Tormented Demons", "More Options");
			stage = 2;
