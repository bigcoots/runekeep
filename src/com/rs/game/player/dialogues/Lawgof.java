...
public class Lawgof extends Dialogue {
	...
	public void completeQuest() {
		if (player.spokeToNu = true && player.fixedRailings == 6 && player.completedDwarfCannonQuest == false && player.getInventory().containsItem(3, 1) && player.getInventory().containsItem(4, 1)) {
			...
			player.questPoints++;
			player.out("Thank you young man! You will now be able to use the Dwarf Multi Cannon! Congratulations. For more information read the Dwarf Cannon quest guide.");
	...
	public void completedRailings() {
			sendNPCDialogue(npcId, 9827, "Thank you very much! I will need one more thing though. A dwarf named Nulodion has my Ammo mould and some notes too, could you bring them to me?");
	...
	public void start() {
		if (player.fixedRailings >= 6 && player.spokeToNu == false) {
			...
			completeQuest();
				...
				sendNPCDialogue(npcId, 9827, "Hello sir! Would you be able to fix my fence? I have counted that "+ player.fixedRailings +"/6 of the railings are fixed, Can you please fix them? I'm sure we can work out a deal after." );
				}
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			sendOptionsDialogue("Would you like to help Captain Lawgof?",
					"Sure thing, hand me the supplies!",
					"No thank you, I'm far too busy.");

		} else if (stage == 0) {
			if (componentId == OPTION_1) {
				if (player.getInventory().getFreeSlots() > 2) {
					sendNPCDialogue(npcId, 9827, "Thank you so much, you will not regret this! Have some supplies.");
					player.getInventory().addItem(14, 6);
					player.getInventory().addItem(2347, 1);
					player.out("I should fix all the broken fences.");
				} else {
					sendNPCDialogue(npcId, 9827, "It seems like you don't have enough inventory space, you need 2 slots.");
				}
				end();
