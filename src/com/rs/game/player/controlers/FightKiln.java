...
public class FightKiln extends Controler {
	...
	public static void enterFightKiln(Player player, boolean quickEnter) {
		...
		if(!player.isCompletedFightCaves()) {
		    player.getPackets().sendGameMessage("Complete Fight Caves before you try entering again.");
			return;
		}
		...
		if(type == 0) {
			...
			if(type == 1 || type == 4) {
				...
				if(type == 4) {
					...
					if (players == null)
						continue;
						players.getPackets().sendGameMessage("<img=6><col=ff0066>News: "+player.getDisplayName()+" Just completed Fightkiln.");
					}
					player.setCompletedFightKiln();
					player.getPackets().sendGameMessage("You were victorious!!");	
						player.getPackets().sendGameMessage("<col=ff0066>Now please logout so your blackscreen is gone.");					
					Integer reward = (Integer) player.getTemporaryAttributtes().get("FightKilnReward");
