...
public class ZombieMinigame extends Controler {
	...
	public static void enterZombieGame(Player player) {
		if(player.getFamiliar() != null || player.getPet() != null || Summoning.hasPouch(player) || Pets.hasPet(player)) {
			player.getDialogueManager().startDialogue("SimpleNPCMessage", ZOMBIE_MONK, "Familiars aren't allowed in the Zombie Minigame!");
			return;
		}
			...
			public void run() {
				...
				WorldTasksManager.schedule(new WorldTask()  {
					...
					public void run() {
						...
						player.getDialogueManager().startDialogue("SimpleNPCMessage", ZOMBIE_MONK, "Goodluck on your mission! <br>Take out as many zombies as you can!");
						player.getPackets().sendGameMessage("To escape from the zombies you must use the door at the east side of the pit.");
	...
	public void setWaveEvent() {
		if(getCurrentWave() == 9) 
			player.getDialogueManager().startDialogue("SimpleNPCMessage", ZOMBIE_MONK, "Look out, here come 2 Skeletal Horrors!");
		CoresManager.fastExecutor.schedule(new TimerTask() {
		...
		if(type == 0 || type == 2)
			...
			if(type == 1 || type == 4) {
				...
				if(type == 4) {
					player.reset();
					player.zombie += 1;
					player.getDialogueManager().startDialogue("SimpleNPCMessage", ZOMBIE_MONK, "You have defeated every wave and recieve a zombie point! You now have "+player.zombie+" zombie points.");
					player.getPackets().sendGameMessage("You were victorious!");
					player.getPackets().sendGameMessage("You got 1 undead token as reward. Spend it at the undead shop at home!");
					 player.getInventory().addItem(19864, 1);
				}else if(getCurrentWave() == 1) 
					player.getDialogueManager().startDialogue("SimpleNPCMessage", ZOMBIE_MONK, "I suppose you tried... better luck next time.");
				else{
