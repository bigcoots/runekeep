 ...
 public class VoteManager {
	...
	public static boolean checkVote(Player p) {
        try {
                ...
                ResultSet rs = query("SELECT status FROM `has_voted` WHERE `username`= '" + playerName + "'");
                if(p.getInventory().getItems().freeSlots() > 0) {
				try {
				String urlString = "http://validus.net/vote/vote.php?user=" + playerName;
